...
if(isitflawed==false){flawless = true;if(bcd==false){lifeAchievements+=1;bcd=true;}}
 ...
 //FOR THE SHOP CODE
 else if(slovakia == 1){image(shop,0,0);
 fill(0,255,0);
 textSize(22);
 ...
 else if(slovakia == 2){image(stats,0,0);
...
if(game==true){
  ...
  if(maceroni == false){
    ...
    if(fieldOrd==0){
    image(field,0,0);}
    else if(fieldOrd==1){image(field2,0,0);}
    else if(fieldOrd==2){image(field3,0,0);}
    else if(fieldOrd==3){image(field4,0,0);}
